...
- ✅ A marked directory's own checkmark clears too, once every file a recursive copy discovered inside it has finished — but only if all of them succeeded (issue #21; before this fix, only leaf filenames were ever cleared, so a marked directory's own mark stayed lit forever). Tracked via a `dirCopyGroup{srcPane, dirName, walkDone, discovered, completed, failed}` per marked top-level directory, referenced directly (no ID/map lookup — see `dirCopyGroup`'s comment) from `transferQueuedMsg`/`transferInfo`/`dirWalkDoneMsg`/`transferQueueErrorMsg` so `completeGroupFile`/`maybeFinalizeGroup` (`internal/ui/app.go`) can tell when the walk is done and every file it found has completed. `BrowserPane.activeCopyGroups` (keyed by directory name) guards against a subtle race: if the user re-marks and re-pushes the same directory name before an earlier push of it finishes, the stale, superseded group's eventual completion must not clear the newer push's mark — `maybeFinalizeGroup` checks pointer identity against `activeCopyGroups[dirName]` before clearing.
...
`Model.transferDest` maps each in-flight transfer ID to a `transferInfo{destPane, srcPane, filename, group}` — which pane ("local"/"remote") it's copying into and out of, which file, and (if it came from a recursive directory copy) a `*dirCopyGroup` back-reference, else `nil`. `TransferDoneMsg` looks up the destination pane and re-lists whatever directory it currently shows, so a completed transfer appears without navigating away and back; it also clears `filename` from the *source* pane's selection, so a marked file's checkmark disappears the moment its own transfer succeeds — other marked files, still in flight, are untouched — and feeds the completion into `completeGroupFile(info.group, false)` for the directory-mark-clearing behavior described above. `TransferErrorMsg` does not touch selection (so a failed transfer stays marked) but does call `completeGroupFile(info.group, true)`, marking that file's directory group as failed. The `transferDest` entry itself is removed on both `TransferDoneMsg` and `TransferErrorMsg`.
